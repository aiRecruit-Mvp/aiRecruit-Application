    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        if (responseBody['user_id'] != null &&
            responseBody['user_id'] is Map<String, dynamic>) {
          final userId = responseBody['user_id']
              ['email']; // Extract the '_id' as the userId
          ...
          Navigator.pushReplacement(
            context,
            MaterialPageRoute(
                builder: (context) => SetPasswordScreen(userId: userId)),
          );
        } else {
          print(
              "The 'user_id' key is not present or not in the expected format.");
        }
      } else {
        print(
